#!/usr/bin/env python3
"""
Generate a TF-IDF benchmark corpus: many rows of ~N words each.
...
Usage (from repository root):
  PGPASSWORD=postgres python3 tf_idf/eval/generate_eval_data.py \
    --table tfidf_eval_data --rows 100000 --words 1000
"""
    ...
    try:
        ...
        while next_doc_id <= num_rows:
            ...
            for doc_id in range(next_doc_id, batch_end + 1):
                ...
                buffer.write(f"{doc_id}\t{document}\n")
            buffer.seek(0)
            cursor.copy_from(buffer, table_name, columns=("doc_id", "txt"))
        ...
        row_count, min_words, max_words = cursor.fetchone()
        print(
            f"Loaded {row_count} rows into {table_name} "
            f"(words/row: min={min_words}, max={max_words})"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Populate a TF-IDF benchmark table with real-word documents"
    )
    parser.add_argument(
        "-t",
        "--table",
        default=os.environ.get("TFIDF_EVAL_TABLE", "tfidf_eval_data"),
        help="Target table name (default: tfidf_eval_data)",
    )
    parser.add_argument(
        "-n",
        "--rows",
        type=int,
        default=env_int("TFIDF_EVAL_ROWS", "100000"),
        help="Number of documents to insert (default: 100000)",
    )
    parser.add_argument(
        "-w",
        "--words",
        type=int,
        default=env_int("TFIDF_EVAL_WORDS", "1000"),
        help="Target words per document (default: 1000)",
    )
    parser.add_argument(
        "-f",
        "--texts-file",
        default=os.environ.get("TFIDF_EVAL_TEXTS_FILE", DEFAULT_TEXTS_FILE),
    ...
    parser.add_argument("--password", default=os.environ.get("PGPASSWORD", DEFAULT_DB_CONFIG["password"]))
