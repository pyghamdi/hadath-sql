...
SELECT 'Riyadh' AS City, * FROM hsql_spatial_partition(5195870.628623282, 2840607.6257099737, 50000);

SELECT 'Dammam' AS City, * FROM hsql_spatial_partition(5572108.243606389, 3051889.530698993, 50000);

SELECT COUNT(*) as "number of cities", hsql_spatial_partition, array_agg(City) AS cities
FROM (
    SELECT 'Makkah' AS City, hsql_spatial_partition(4433432.30, 2442329.09, 500000)
    UNION ALL
    SELECT 'Jeddah' AS City, hsql_spatial_partition(4362889.14, 2449900.21, 500000)
    UNION ALL
    SELECT 'Riyadh' AS City, hsql_spatial_partition(5195870.62, 2840607.62, 500000)
    UNION ALL
    SELECT 'Dammam' AS City, hsql_spatial_partition(5572108.24, 3051889.53, 500000)
    UNION ALL
    SELECT 'Almadina' AS City, hsql_spatial_partition(4409832.57, 2810613.91, 500000)
)
...
SELECT
    ...
    x,
    y,
    hsql_spatial_partition(x, y, 50000) AS partition
